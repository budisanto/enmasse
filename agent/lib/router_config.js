    ...
    return retrieve_config(actual, router).then(function (errors) {
        ...
        entities.filter(retrieved_without_error).forEach(function (entity) {
            ...
            if (delta) {
                ...
                log.debug('updating configuration of %s on %s: stale=%j, missing=%j', entity.name, router_id, stale[entity.name], missing[entity.name]);
            } else {
        ...
        if (missing.size() || stale.size() || Object.keys(errors).length) {
            ...
            return work.then(function () {
                ...
                log.error('error while applying address configuration to %s, retrying: %j', router_id, error);
                return apply_config(desired, router);
            });
        } else {
